...
CAPTION_TRAIN_JSON_FNAME = os.path.join(
    DATA_ROOT_PATH, 'json', 'yfcc-caption-train.json'
)
CAPTION_TEST_JSON_FNAME = os.path.join(
    DATA_ROOT_PATH, 'json', 'yfcc-caption-test.json'
)
...
def tokenize(sentence):
  ...
  sentence = re.sub(r"(?<![a-zA-Z])0(?![a-zA-Z])", r"", sentence)  # remove "digit" if there is no preceed or following
  ...
  # Escape unicode
  sentence = sentence.encode('unicode-escape').decode('unicode-escape').encode('ascii', 'ignore')

  return sentence.split()


def tokenize_all(train_json, test_json, key='caption'):
  """
  ...
  for user_id, posts in tqdm(train_json.items(), ncols=70, desc="train data"):
    ...
    for post in posts:
      ...
      if len(title) >= 3 and len(title) <= 20:
        post_tokens = title
      elif len(description) >= 3 and len(description) <= 20:
        post_tokens = description
      else:
        post_tokens = title
...
def save_data(train_data, test_data, output_path, rev_vocab):
  """
  Data format:
...
def main():
  ...
  caption_test_json = load_json(CAPTION_TEST_JSON_FNAME)

  # Tokenize all
  caption_counter, caption_train_tokens, caption_test_tokens = tokenize_all(
          caption_train_json,
          caption_test_json,
          'caption'
      )
  ...
  with open('counter.txt', 'w') as f:
    for key, value in caption_counter.most_common():
      f.write("%s : %d\n" % (key, value))
  ...
  caption_train_tfidf_tokens, caption_test_tfidf_tokens = get_tfidf_words(
  ...
  save_data(
      ...
      (caption_test_tokens, caption_test_tfidf_tokens),
      CAPTION_OUTPUT_PATH,
      caption_rev_vocab
  )
